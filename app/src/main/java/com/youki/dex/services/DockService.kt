...
import com.youki.dex.activities.LAUNCHER_PAUSED
...
class DockService : AccessibilityService(), OnSharedPreferenceChangeListener, OnTouchListener,
    ...
    override fun onServiceConnected() {
        ...
        fun animateBtn(view: View, action: () -> Unit) {
            val pressAnim = AnimationUtils.loadAnimation(context, R.anim.btn_press)
            val releaseAnim = AnimationUtils.loadAnimation(context, R.anim.btn_release)
            pressAnim.setAnimationListener(object : Animation.AnimationListener {
    ...
    private fun showDock() {
        ...
        dockLayout.scaleX = 0.92f
        dockLayout.scaleY = 0.92f
        dockLayout.alpha = 0f
    ...
    private fun getDefaultLaunchMode(app: String?): String {
        // Always floating override
        if (sharedPreferences.getBoolean("always_floating", false)) return "standard"
        if (app == null)
            return "standard"
        ...
        return if (sharedPreferences.getBoolean("remember_launch_mode", true) && mode != null)
            ...
            sharedPreferences.getString("launch_mode", "standard") ?: "standard"
    }
    ...
    private fun setOrientation() {
        val lockLandscape = sharedPreferences.getBoolean("lock_landscape", false)
    ...
    fun showAppMenu() {
        ...
        appMenu.animate()
            ...
            .setListener(object : AnimatorListenerAdapter() {
                ...
                override fun onAnimationCancel(animation: android.animation.Animator) {
                    appMenu.scaleX = 1f; appMenu.scaleY = 1f; appMenu.alpha = 1f
                    appMenu.setLayerType(View.LAYER_TYPE_NONE, null)
                }
            }).start()
    ...
    private fun updateRunningTasks(recreateAdapter: Boolean = false) {
        ...
        // Let RecyclerView size itself naturally — it's already horizontal+scrollable
        ...
        if (adapter is DockAppAdapter && !recreateAdapter)
            ...
            tasksGv.adapter = DockAppAdapter(context, apps, this, iconPackUtils, dockHeight)
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        // DPI أو display تغير — أعد حساب الأحجام
        updateDockHeight()
        updateDockShape()
    ...
    inner class HotCornersHoverListener(val key: String) : View.OnHoverListener {
        override fun onHover(
            ...
            if (event.action == MotionEvent.ACTION_HOVER_ENTER) {
                ...
                }, sharedPreferences.getString("hot_corners_delay", "300")?.toLongOrNull() ?: 300L)
            }
            return false
        }
    }
