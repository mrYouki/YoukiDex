            ...
            if (!isNotificationOn) {
                showDialog(
                    android.app.AlertDialog.Builder(this)
                        .setTitle("صلاحية الإشعارات مطلوبة")
                        .setMessage("يحتاج YoukiDex إذن الوصول للإشعارات حتى تعمل بشكل صحيح. سيتم توجيهك لتفعيله.")
                ...
                if (hasPermission) {
                    ...
                    android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                        if (DeviceUtils.isAccessibilityServiceEnabled(applicationContext)) {
                            ...
                            showDialog(
                                android.app.AlertDialog.Builder(this)
                                    .setTitle("تفعيل خدمة Accessibility مطلوب")
                                    .setMessage("تعذّر تفعيل الخدمة تلقائياً. سيتم توجيهك لتفعيلها يدوياً من الإعدادات.")
                                    .setPositiveButton("حسناً") { _, _ ->
                                        launchActivity(
                    ...
                    showDialog(
                        android.app.AlertDialog.Builder(this)
                            .setTitle("تفعيل خدمة Accessibility مطلوب")
                            .setMessage("يحتاج YoukiDex تفعيل خدمة Accessibility للعمل. سيتم توجيهك للإعدادات.")
                            .setPositiveButton("حسناً") { _, _ ->
                                launchActivity(
                                    Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)
                                        .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
