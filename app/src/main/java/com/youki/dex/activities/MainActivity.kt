...
import com.youki.dex.utils.ShellManager
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // عرض زر "أضف اختصار لسطح المكتب" لو لسه ما اضيف
    ...
    // Auto-grant WRITE_SECURE_SETTINGS if Shizuku/ShellServer available, else show setup
    private fun handleWriteSecureSettings() {
        val shell   = ShellManager.getInstance(this)
        ...
        when {
            ...
            shell.isConnected -> {
                shell.runShell("pm grant $pkg android.permission.WRITE_SECURE_SETTINGS") { result ->
                    runOnUiThread {
            ...
            else -> showShellSetupDialog()
        }
    }

        // Shell Setup Dialog - works via ShellServer or Shizuku
    ...
    private fun showShellSetupDialog() {
        val shell   = ShellManager.getInstance(this)
        val shizuko = ShizukoManager.getInstance(this)
        ...
        refreshSetupState(shell, shizuko)

        // ShellManager callbacks
        shell.onLog         = { msg -> appendLog(msg) }
        shell.onConnected   = { _ ->
            runOnUiThread {
                setupStatusTv.text = "ShellServer connected"
                ...
                setupActionBtn.setOnClickListener {
                    ...
                    ) { result -> appendLog(result); updatePermissionsStatus() }
                }
            }
            updatePermissionsStatus()
        }
        shell.onDisconnected = {
            runOnUiThread {
                setupStatusTv.text = "ShellServer disconnected"
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setNegativeButton(R.string.ok) { _, _ ->
                shell.onLog         = null
                shell.onConnected   = null
                shell.onDisconnected = null
                shizuko.onLog       = null
    ...
    private fun refreshSetupState(shell: ShellManager, shizuko: ShizukoManager) {
        when {
            shell.isConnected -> {
                setupStatusTv.text = "ShellServer connected"
                setupStatusTv.setTextColor(0xFF4CAF50.toInt())
            ...
            shizuko.hasPermission -> {
                ...
                setupStatusTv.text = "Not connected - need ShellServer or Shizuku"
                setupStatusTv.setTextColor(0xFFEF5350.toInt())
                setupActionBtn.text = "Connect to ShellServer"
                setupActionBtn.isEnabled = true
                setupActionBtn.setOnClickListener {
                    ShellManager.getInstance(this).connect()
                }
            }
        }
    }
