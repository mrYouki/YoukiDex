    ...
    private fun showDisplaySizeDialog(context: Context) {
        ...
        dialog.setPositiveButton(R.string.ok) { _, _ ->
            ...
            if (shizuku.hasPermission) {
                val cmd = if (value.isEmpty() || value == "0") "wm density reset" else "wm density $value"
                shizuku.runShell(cmd) { result ->
                    android.widget.Toast.makeText(
                        context,
                        if (result.contains("error", ignoreCase = true)) "Failed: $result" else "Display density applied!",
                        android.widget.Toast.LENGTH_SHORT
                    ).show()
                }
            } else {
                // Fallback: write secure setting + reboot
                val size = if (value == "0") "" else value
                if (DeviceUtils.putSecureSetting(context, DeviceUtils.DISPLAY_SIZE, size) && rootAvailable)
